...
public class Spawn {
    ...
    public void tick() {
        ...
        if (timer == 0) {
            handler.addObject(new BasicEnemy(r.nextInt(Game.WIDTH), 0, ID.BasicEnemy, handler, hud));
            timer = 30;
        } else
            timer--;
        ...
        if (scoreKeep >= 500) {
            ...
            if (hud.getLevel() % 1 == 0) {
                handler.addObject(new SmartEnemy(r.nextInt(Game.WIDTH), 0, ID.SmartEnemy, handler));
                // }
                // if (hud.getLevel() == 4) {
